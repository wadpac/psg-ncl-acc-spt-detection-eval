...
pathpsg = "/media/vincent/Exeter/psg_study/cleaned_psg_penn"
pathacc = "/media/vincent/Exeter/psg_study/cleaned_acc_penn"
pathparticipantinfo = "/media/vincent/Exeter/Geneactiv validation data/participants_diagnosis.csv"
...
psgR = merge(comppsg,compacc,by="id")
...
pdf(file=paste0("/media/vincent/Exeter/Supplement_psg_accelerometer_penn.pdf"),width = 7,height = 3.5)
for (location in c("right")) {
  ...
  for (h in 1:nrow(psgdata)) { # loop through all psg recordings
    ...
    PSG$time = as.POSIXlt(PSG$time,origin="1970-1-1",tz = "America/New_York")
    ACC$timestampPOSIX = as.POSIXlt(ACC$timestampPOSIX,origin="1970-1-1",tz = "America/New_York")
    PSG = PSG[-which(PSG$Event %in% c("Diagnostic", "Lights Off", "Supine", "Right", "Lights On") == TRUE),]
    ...
    PSG$time = as.POSIXlt(seq(starttime,starttime+(nrow(PSG)*5)-1,by=5),origin="1970-1-1",tz = "America/New_York")
    ...
    psgacc_expand$time = psgacc_expand$timestampPOSIX = as.POSIXlt(psgacc_expand$timenum,origin="1970-1-1",tz = "America/New_York")
    ...
    if (length(sptwindow$lightson) != 0 & length(sptwindow$lightsout) != 0) {
      ...
      output$est_PSTonset[h] = strftime(as.POSIXlt(sleep_est$time[sptwindow$lightsout], origin="1970-1-1", tz="America/New_York")[1], format='%H:%M:%S')
      output$est_PSTwake[h] = strftime(as.POSIXlt(sleep_est$time[sptwindow$lightson], origin="1970-1-1", tz="America/New_York")[1], format='%H:%M:%S')
      # True sleep according to PSG
      
      psgacc$stagescore[which(psgacc$Event == "REM")] = 1
      psgacc$stagescore[which(psgacc$Event == "NREM 1")] = 2
      psgacc$stagescore[which(psgacc$Event == "NREM 2")] = 3
      psgacc$stagescore[which(psgacc$Event == "NREM 3")] = 4
      ...
      output$true_PSTonset[h] = as.character(psgacc$Start.Time[which(pss != 0)[1]])
      output$true_PSTwake[h] = as.character(psgacc$Start.Time[(length(pss)-match(1,rev(psstmp))+1)])
  ...
  convchartime2number = function(x) {
    gettime = function(y) {
      if (is.na(y) == FALSE) {
        y = unlist(strsplit(y," ")[1]) #remove pm/am
...
if (simulate24 ==  FALSE) {
  ...
  plot(output_right$true_PSTdur[norm],meth_diff[norm],xlab="PSG (hours)",
       ylab="HDCZA  - PSG (hours)",main="SPT-window duration",
  ...
  plot(output_right$true_sleepdur[norm],meth_diff[norm],xlab="PSG (hours)",
       ylab="HDCZA  - PSG (hours)",main="Sleep duration",
